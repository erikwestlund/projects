...
tmux new-window -t $SESSION_NAME:2 -n "co-mini" -c "$HOME/code/presentations" /bin/zsh
...
tmux new-window -t $SESSION_NAME:4 -n "co-max" -c "$HOME/code/presentations" /bin/zsh
tmux send-keys -t $SESSION_NAME:4 "codex --model gpt-5.1-codex-max -c model_reasoning_effort=\"high\"" C-m

tmux new-window -t $SESSION_NAME:5 -n "co-gpt" -c "$HOME/code/presentations" /bin/zsh
tmux send-keys -t $SESSION_NAME:5 "codex --model gpt-5.1 -c model_reasoning_effort=\"high\"" C-m

# Create R console window (window 6)
tmux new-window -t $SESSION_NAME:6 -n "R" -c "$HOME/code/presentations" /bin/zsh
tmux send-keys -t $SESSION_NAME:6 "R" C-m

# Create Python console window (window 7)
tmux new-window -t $SESSION_NAME:7 -n "Python" -c "$PROJECT_DIR" /bin/zsh
tmux send-keys -t $SESSION_NAME:7 "python3" C-m
