...
# Create Codex window (index 3)
tmux new-window -t $SESSION_NAME:3 -n codex -c "$PROJECT_DIR" /bin/zsh
tmux send-keys -t $SESSION_NAME:3 "codex" C-m

# Create Tinker window (index 4)
tmux new-window -t $SESSION_NAME:4 -n tinker -c "$PROJECT_DIR" /bin/zsh
tmux send-keys -t $SESSION_NAME:4 "php artisan tinker" C-m

# Create Horizon window (index 5)
tmux new-window -t $SESSION_NAME:5 -n horizon -c "$PROJECT_DIR" /bin/zsh
tmux send-keys -t $SESSION_NAME:5 "php artisan horizon" C-m

# Create NPM window (index 6)
