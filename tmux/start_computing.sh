...
if [ $? != 0 ]; then
    ...
    tmux new-window -t $SESSION_NAME:3 -n "ha" -c "$HOME/code/home-assistant" /bin/zsh
    
    # Create window for homelab (window index 4)
    tmux new-window -t $SESSION_NAME:4 -n "homelab" -c "$HOME/code/homelab" /bin/zsh
    
    # Create window for vscode (window index 5)
    tmux new-window -t $SESSION_NAME:5 -n "vscode" -c "$HOME/code/vscode" /bin/zsh
    
    # Create window for raycast (window index 6)
    tmux new-window -t $SESSION_NAME:6 -n "raycast" -c "$HOME/code/raycast" /bin/zsh
